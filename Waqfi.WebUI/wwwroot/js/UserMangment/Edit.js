...
function GetRole() {
    ...
    $.ajax({
        ...
        var AssignRoles = $('#UserRole').val();

        var array=[]

         array = AssignRoles.split(',');

        $.each(data, function (key, dataval) {
            var option = new Option(dataval.descriptionAr, dataval.id, true, false);
                    RoleSelect.append(option).trigger('change');

        });

        RoleSelect.val(array);
...
function EditUser() {
    ...
    var GetPhone = $("#Phone").val().trim().trim();
    ...
    $.ajax({
        ...
        contentType: 'application/x-www-form-urlencoded',
        dataType: "json",
        data: { "UserId": GetUserId, "UserName": GetUserName, "Email": GetEmail, "Phone": GetPhone, "Roles": GetRoles },
        headers:
        {
            "RequestVerificationToken": $('input:hidden[name="__RequestVerificationToken"]').val()
        },
        success: function (html) {
            ...
            if (IfSucces)
            {
                ...
                $('#TextValidation').text(html.message)
            }
        }
    });
}
...
function swl() {
        Swal.fire({
            imageUrl: `/Cms_Lib/img/icons/check-done.svg`,
            html: `<strong style='font-weight: bold'> تمت عملية التعديل بنجاح</strong>`,
            showCancelButton: false,
            timer: 3000
        }).then((result) => {
            debugger
            window.location.href = "/UserMangment/Index";
